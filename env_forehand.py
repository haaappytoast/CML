...
from env import HumanoidTennisVRControl
import random
...
class ICCGANHumanoidForehand(HumanoidTennisVRControl):
    ...
    def _load_proj_asset(self):
        self._proj_handles = []
        asset_root = "./assets"
        small_asset_file = "tennis_marker.urdf"
        ...
        print("===== loaded tennis_marker asset =====")
        return
    ...
    def _build_proj_tensors(self):
        ...
        # 캐릭터 num_bodies 이후, 마지막 원소만 가져오기 [1, n_objs, 3]    
        self._proj_contact_forces = contact_force_tensor.view(self.num_envs, bodies_per_env, 3)[..., -num_objs:, :]
        return
    ...
    def _update_proj(self):
        ...
        tar_body_idx = self.tar_body_idx[0]
        dir_body_idx = self.dir_body_idx[0]
    ...
    def reaach(self):
        ...
        self._proj_states[..., perturb_id, 0] = humanoid_root_pos[..., 0] + ego_offset_vec[..., 0]
        self._proj_states[..., perturb_id, 1] = humanoid_root_pos[..., 1] + ego_offset_vec[..., 1]
        self._proj_states[..., perturb_id, 2] = humanoid_root_pos[..., 2] + self.map_z(self.shoot)[2]
        #self._proj_states[..., 0:3] = humanoid_root_pos[..., 0:3] + ego_offset_vec[..., 0:3]
        self._proj_states[..., 9] = 0
        self.gym.set_actor_root_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(self._all_root_tensor),
                                                    gymtorch.unwrap_tensor(self._proj_actor_ids),
                                                    len(self._proj_actor_ids))
        return
    ...
    def update_obj_actions(self, event):
        # reach
        if (event.action == "first") and event.value > 0:
            self.reach = True
            self.shoot = "first"
        if (event.action == "second") and event.value > 0:
            self.reach = True
            self.shoot = "second"                        
