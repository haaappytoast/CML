...
class Env(object):
    ...
    def update_viewer(self):
        ...
        for event in self.gym.query_viewer_action_events(self.viewer):
            ...
            # projectile
            elif (event.action == "right_shoot" or event.action == "left_shoot") and event.value > 0:
                self.perterb = True
    ...
    def step(self, actions):
        if not self.viewer_pause or self.viewer_advance:
            ...
            self.do_simulation()
            if "Projectile" in str(self.__class__):
...
class ICCGANHumanoidVR(ICCGANHumanoidEE):  
    ...
    def reset_envs(self, env_ids):
        ...
        temp_root = self.root_tensor.clone().squeeze(1)
        self.gym.set_actor_root_state_tensor_indexed(self.sim,
            gymtorch.unwrap_tensor(temp_root),
...
class ICCGANHumanoidProjectile(ICCGANHumanoidVRControl):
    ...
    def _update_proj(self):
        ...
        return        
    def setup_sim_params(self):
        sim_params = super().setup_sim_params()
